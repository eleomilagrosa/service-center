...
public class ShippingActivity extends BaseActivity implements
    ...
    public final static int FRAGMENT_JOB_ORDER_SHIPPING = 2;
    ...
    public final static int REQUEST_CREATE_SHIPPING = 301;
    ...
    protected void onCreate(Bundle savedInstanceState){
        ...
        jobOrders.setJobOrderShipping(new JobOrderShipping());
        jobOrders.getJobOrderShipping().setJob_order_id(jobOrders.getId());
    ...
    public void onBackPressed(){
        ...
        }else if(current_fragment == FRAGMENT_JOB_ORDER_SHIPPING){
            alertToExit();
        }else if(current_fragment == FRAGMENT_JOB_ORDER_IMAGES){
            switchFragment(FRAGMENT_JOB_ORDER_SHIPPING);
    ...
    public void switchFragment(int fragment){
        ...
        switch (fragment){
            case FRAGMENT_JOB_ORDER_SHIPPING:
                ShippingFragment shippingFragment = new ShippingFragment();
                shippingFragment.setJobOrder(jobOrders);
                setFragment(shippingFragment,before_fragment,current_fragment);
                break;
    ...
    public void setJobOrderImage(JobOrderImages jobOrderImage,String path){
        ...
        jobOrderImage.setJob_order_status_id(JobOrders.ACTION_FORWARDED);
    }
    public void save(){
        showProgress(true);
        create_job_order_shipping();
    }
    public void create_job_order_shipping(){
        requestManager.setRequestAsync(requestManager.getApiService().create_job_order_shipping(jobOrders.getId(),jobOrders.getJobOrderShipping().getShipping_no(),jobOrders.getJobOrderShipping().getShipping_note(),jobOrders.getJobOrderShipping().getAccount_id()),REQUEST_CREATE_SHIPPING);
    }
                ...
                if(!jobOrders.isEmpty()){
                    realm.executeTransaction(new Realm.Transaction(){
                        ...
                        public void execute(Realm realm){
                            ShippingActivity.this.jobOrders.getJobOrderImageslist().remove(ShippingActivity.this.jobOrders.getJobOrderImageslist().size()-1);
                            jobOrders.get(0).getJobOrderImages().addAll(ShippingActivity.this.jobOrders.getJobOrderImages());
                            jobOrders.get(0).getJobOrderImages().addAll(ShippingActivity.this.jobOrders.getJobOrderImageslist());
